...
$$Y_t = C_0 + B_1Y_{t-1} + B_2Y_{t-1} + ... + B_pY_{t-p} + u_t$$ with
$u_t \sim N(0,\Sigma_0)$.
...
The following codes demonstrate how var_data is used to generate data
from a VAR(p) process with a variety of specifications.

``` r
res_d = var_data(n=2,p=2,T=100,type="const")
res_d = var_data(n=2,p=2,T=100,Co=c(1:2)*0,type="none")
res_d = var_data(n=2,p=2,T=100,Co=c(1:2),  type="const")
res_d = var_data(n=3,p=2,T=200,type="exog1",X=matrix(rnorm(400),200,2))
res_d = var_data(n=3,p=2,T=200,Co=matrix(c(0,0,0,1,2,3,3,2,1),3,3), type="exog0",X=matrix(rnorm(400),200,2))
```

var_data can be used to generate data for given parameters. If
parameters are not provided var_data will generate data by default with
randomly chosen parameters.

The output of var_data is a VAR object containing generated data, used
...
The var_estimate function is used to estimate the parameters for the
given data.
...
The output of var_estimate is a VAR object with estimated parameters.

For empirical applications it is recommended to follow the steps below:

- Use var_data to generate a VAR object.
...
- Run a model selection procedure to determine the specification of the
  model if necessary.
- Use var_data to re-generate a VAR object with the selected
...
res_d = var_data(n=4,p=2,T=84,Co=matrix(c(0,0,0,0),4,1),type="none")
...
res_d = var_data(n=4,p=2,T=84,Co=matrix(c(1,1,1,1),4,1),type="const")
res_d$Y = as.matrix(Canada)
res_e = var_estimate(res=res_d)
res_e$Summary$BIC
res_e$Summary$AIC
...
res_d = var_data(n=4,p=2,T=84,Co=matrix(c(0,0,0,0,1,1,1,1),4,2),type="exog0",X=trend)
res_d$Y = as.matrix(Canada)
res_e = var_estimate(res=res_d)
res_e$Summary$BIC
res_e$Summary$AIC

res_d = var_data(n=4,p=2,T=84,Co=matrix(c(1,1,1,1,1,1,1,1),4,2),type="exog1",X=trend)
res_d$Y = as.matrix(Canada)
res_e = var_estimate(res=res_d)
res_e$Summary$BIC
res_e$Summary$AIC
...
package $vars$. In particular, the function VARselect of $vars$ can be
used to specify the lag length of a VAR model.
...
IRF_list <-plot_irf(IRF_CB, response=c(1,2,3),Names=c("Y1","Y2","Y3"),IName=c("X1","X2"), impulse=c(1,2),ncol=2)
```
...
follows.
$$Y_t = C_0 + B_1Y_{t-1} + B_2Y_{t-1} + ... + B_pY_{t-p} + u_t$$ Because
of unit roots and cointegration relations in the VAR process, it has the
following vector error correction form (VECM):
$$\Delta Y_t = C_0 + \alpha\beta' Y_{t-1} + B^*_1\Delta Y_{t-1} + ... + B^*_{p-1}\Delta Y_{t-p+1} + u_t$$
where $\alpha$ and $\beta$ are $(n\times ckr)$ matrices and
$u_t \sim N(0,\Sigma_0)$.
...
The following codes demonstrate how civar_data can be used to generate
data from a cointegrated VAR process with a variety of specifications.

``` r
res_d = civar_data(n=2,p=2,T=100,type="const")
res_d = civar_data(n=3,p=2,T=100,Co=c(1:2)*0,type="none")
res_d = civar_data(n=4,p=1,T=100,Co=c(1:2)*NA,type="const",crk=2)
spectral_radius(res_d$B)
```

The output of civar_data is a CIVAR object containing the generated
data, the used parameters as well as the residuals. The components
correspond to the model variables and parameters as follows.
...
civar_data differs from var_data in that the data generating process
contains unit-roots. The spectral_radius function calculates the roots
of characteristic polynomial in lags or equivalently the eigenvalues of
the companion matrix of the CIVAR model.

The default number of unit-root in civar_data is one. The option of r_np
matrix can be used to specify the number of unit roots.

civar_estimate is used to estimate the parameters of the generated CIVAR
data.
...
res_d  = civar_data(n=4,p=3,T=500,r_np=r_np,Co=matrix(0,n,1) ,type="none",crk=3)
res_e  = civar_estimate(res=res_d)
res_e$Summary


B0 = res_d$B
res_d  = civar_data(n=4,p=3,T=5000,B = B0,Co=matrix(0,n,1) ,type="none",crk=3)
res_e  = civar_estimate(res=res_d)
res_e$Summary
...
- Use civar_data to generate a CIVAR object.
- Replace the generated model data with the empirical data.
- Run a model selection and testing procedure to determine the
  specification of the model.
- Use civar_data to generate a CIVAR object with the selected
...
res_d = civar_data(n=4,p=2,T=84,type="const",crk=2)
res_d$Y = Canada
plot(ts(res_d$Y))
res_e = civar_estimate(res=res_d)
res_e$Summary
...
res_d = civar_data(n=4,p=2,T=84,type="const",crk=1)
res_d$Y = Canada
res_e = civar_estimate(res=res_d)
res_e$Summary
...
# step 3 Model selection based on AIC or BIC
res_d2 = civar_data(n=4,p=2,T=84,Co=matrix(c(1,1,1,1),4,1)*0,type="none",crk=1)
res_d2$Y = as.matrix(Canada)
res_e2 = civar_estimate(res=res_d2)
res_e2$Summary$BIC
res_e2$Summary$AIC
...
IRF_list <-plot_irf(IRF_CB,Names=c("e","prod","rw","U"))
```

### Test of restrictions on $\alpha$ and $\beta$

civar_test function calculates maximum likelihood tests on the
restrictions within the cointegration space i.e. restrictions on $\beta$
and $\alpha$. The restrictions are formulated as follows.
...
$\alpha$ and $\beta$. The likelihood ratio value is zero.
...
res_d = civar_data(n=3,p=2,T=T,Co=matrix(c(2,2,2,1,2,3,3,2,1),3,3), type="exog1",X=X) ;   res_e = civar_estimate(res=res_d);res_e$Summary
...
ccivar_data and ccivar_estimate are functions used to model conditional
...
mixed_civar_data function is used to generate CIVAR data with mixed I(1)
and I(0) variables. It is also used to estimate the mixed CIVAR models.
The following example codes generate a set of data from a mixed CIVAR
...
$$ \beta' =\left(
  ...
  \end{array}
\right)
  $$ $B^{33}$ iS a (2 x 2) submatrix corresponding to the two I(0)
variables . $(B^{21},B^{22})$ is a (2 x 7) cointegration matrix of the 7
I(1) variables. These restrictions on $\beta$ can be tested using
civar_test
...
RR$GABtest$LR
### The null of mixed CIVAR is not rejected for mixed data generated by mixed_civar_data
```
...
res_d = civar_data(n=4,p=2,T=84,type="none",crk=2)
res_e = civar_estimate(res=res_d)
res_e$Summary
...
where $u^{(k)}_t \sim N(0,\Sigma_0^{(k)})$ for $k= 1,2$. This model is
also called threshold VAR model as the choice of regime depends on the
threshold condition $f_{t-d}\le \tau$. Often $f_t$ is one component of
the endogenous variable $Y_t$. In this case, it is called self-exiting
threshold VAR model.
...
Following sample codes demonstrate how mrvar_data is used to generate
...
The output of mrvar_data is an MRVAR object containing the generated
...
res_d\$B_0:
$B^{(1)}_1,B^{(1)}_2,...,B^{(1)}_p;B^{(2)}_1,B^{(2)}_2,...B^{(2)}_p$

res_d\$Sigmao: $\Sigma^{(1)}_0,\Sigma^{(2)}_0$

res_d\$Uo:
$(u^{(1)}_1,u^{(1)}_2,...,u^{(1)}_T;u^{(2)}_1,u^{(2)}_2,...,u^{(2)}_T)'$
...
\left(
  \begin{array}{cc}
    ...
    1 & 0 \\
  \end{array}
\right)$ specifies that the lag for the first regime is 2 and there is
no exogenous variables, while for the second regime the lag length is 1
and there is no exogenous variables too. The function mrvar_estimate is
used to estimate the parameters of the generated data.
...
res_e = mrvar_estimate(res=res_d)
  ...
  #x11()
  #IRF_list <-plot_irf(RF4,Names =c("R","P") )
...
res_d = mrvar_data(n=4,p=p,T=800,S=2,SESVI=1,TH=0,Sigmao=Sigma,type="none")
max(res_d$Y)
res_e = mrvar_estimate(res=res_d)
res_e$Summary
...
mrvar_select\_ calculates AIC and BIC for different lag lengths in
...
Option X in the function mrvar_data is used to construct MRVAR models
...
res_d = mrvar_data(n=2,p=p,T=300,S=2,SESVI=1,type="exog1",X=XX)
#max(res_d$Y)
res_e = mrvar_estimate(res=res_d)
#res_e$Summary
...
- Use mrvar_data to generate an MRVAR object.
- Replace the generated model data with the empirical data.
- Estimate model parameters using mrvar_estimate
- Specify the the threshold value and the lag length using
  mrvar_select\_ if necessary.
...
where $u^{(k)}_t \sim N(0,\Sigma_0^{(k)})$ for $k= 1,2$. Because of unit
roots and cointegration relations the MRCIVAR has a vector error
correction form:
...
where $\alpha^{(1)}$, $\alpha^{(2)}$ and $\beta$ are $(n\times ckr)$
matrices. Usually the cointegration relation $\beta$ is assumed to be
identical in different regimes while the adjustment speed $\alpha^{(k)}$
...
The following sample codes demonstrate how mrcivar_data can be used to
...
parameters. mrcivar_select calculates AIC or BIC criteria for a range of
parameter settings.
...
res_d = mrcivar_data(n=4,p=p,T=261,S=2,SESVI=1,TH=0,Sigmao=Sigma,type="const",r=1)
...
\left(
  \begin{array}{cc}
    ...
    2 & 0 \\
  \end{array}
\right)$ specifies that the first regime has a lag length of 3 and no
exogenous variables, while the second regime has a lag length of 2 and
no exogenous variables. MRCIVARestm produces an MRCIVAR object
containing estimated results and test statistics.
...
dy2.2, dy3.2, and dy4.2 denote $\Delta y_{1,t-1}$, $\Delta y_{2,t-1}$,
$\Delta y_{3,t-1}$, $\Delta y_{4,t-1}$, $\Delta y_{1,t-2}$,
$\Delta y_{2,t-2}$, $\Delta y_{3,t-2}$, and $\Delta y_{4,t-2}$. They are
the regime 1 regressors. In the second block, dy1.1, dy2.1, dy3.1, and
dy4.1 represent the regime 2 regressors $\Delta y_{1,t-1}$,
$\Delta y_{2,t-1}$, $\Delta y_{3,t-1}$, and $\Delta y_{4,t-1}$. Because
regime 2 has two lags in level VAR, the error correction form has only
one lag. The result of the Johansen test of the cointegration rank is
also displayed in res_e\$Summary.

b2_cib can be used to calculate the cointegration vectors and the
...
spectral_radius(res_d$Bo[,,,1])
spectral_radius(res_d$Bo[,,,2])
```

For empirical applications it is recommended to follow the steps:

- Use mrcivar_data to generate a MRCIVAR object.
- Replace the generated model data with the empirical data.
- Estimate the model
- Run analysis.
...
res_d = mrcivar_data(n=4,p=p,T=260,S=2,SESVI=1,TH=0,Sigmao=Sigma,type="const",r=1)
res_e = mrcivar_estimate(res=res_d)
...
BIC will be calculated. The mrvar_select\_\_summary function is used to
...
where $y_{i,t}$ is an $m$-vector of domestic variables with
$y_{i,t}=(y_{i,1,t},y_{i,2,t},...,y_{i,m,t})'$ and $y_{i,t}^{*}$ is an
$m$-vector of the corresponding foreign variables with
$y_{i,t}^{*}=(y_{i,1,t}^{*},y_{i,2,t}^{*},...,y_{i,m,t}^{*})'$,
$i=1,2,...,n$. $n$ is the number of countries. Stacking all $n$ country
models together we have GVAR in its global form:
...
The following sample codes demonstrate how gvar_data is used to generate
...
The output of gvar_data is a GVAR object containing generated data and
...
res_d\$Bo:
$B_{1,1},...,B_{1,p},B_{2,1},...,B_{2,p},...,B_{n,1},...B_{n,p}$

res_d\$Ao:
$A_{1,1},...,A_{1,q},A_{2,1},...,A_{2,q},...,A_{n,1},...A_{n,q}$
...
\left(
  \begin{array}{ccc}
    ...
    2 & 1 & 0 \\
  \end{array}
\right)$ specifies that for country 1 the lag length of the domestic
variables is 2 and that of the foreign variables is also 2. For
countries 2 to 5, the lag lengths of domestic variables are 2 and those
of the foreign variables are 1. The output of gvar_estimate, res_e, is a
GVAR object containing estimated results and test statistics.
res_e\$Summary gives the country model regression results and test
statistics.
...
the foreign variables: $Y_{4,1,t-1}$, $Y_{4,2,t-1}$, $Y_{4,1,t-2}$,
$Y_{4,2,t-2}$, $Y_{4,1,t-1}^{*}$, $Y_{4,2,t-1}^{*}$,respectively.

Option X in gvar_data can be used to include exogenous variables in GVAR
...
gvar_select\_ calculates AIC and BIC to lags up to the upper bounds
given in L_V for country I.
...
L_V = c(4,4)
res_d$p
GVARSelect = gvar_select_(res=res_d,L_V=c(4,4),I=1)
...
L_V = c(4,4)
res_d$p
GVARSelect = gvar_select_(res=res_d,L_V=c(4,4),I=3)
GVARSelect[which.min(GVARSelect[,3]),]
```

For empirical applications it is recommended to follow the steps below:

- Use gvar_data to generate a GVAR object.
- Replace the generated model data with the empirical data.
- Run a model selection procedure to determine the specification of the
...
Stacking all $n$ country models together we have the CIGVAR in its
global form:
$$Y_{t}=C+\sum_{l=1}^{P}G_{l}Y_{t-l}+\epsilon_t$$
with $\epsilon_t \sim N(0,\Sigma)$,
$$Y_{t}=(y_{1,t}',y_{2,t}',...,y_{n,t}')'$$
...
cigvar_data creates a CIGVAR object with provided or randomly generated
...
cigvar_estimate estimates the parameters of a CIGVAR object.  
...
p = (1:15)*0; dim(p) = c(5,3)
p[,1] = 3; p[,2]=3;
res_d = cigvar_data(m=4,n=5,p=p,T=1000,type="const")
max(abs(res_d$Y))
...
The output of cigvar_data is a CIGVAR object containing generated data
and parameters as well as the residuals. The components of the generated
cigvar_data object correspond to the model variables and parameters as
follows.

res_d\$Y: $Y = (Y_1,Y_2,...,Y_T)'$,
$Y_t = (y_{1,1,t},y_{1,2,t},...,y_{1,m,t},y_{2,1,t},y_{2,2,t},...y_{2,m,t},...,y_{n,m,t})'$

res_d\$Uo: $U_o = (U_1,U_2,...,U_T)'$,
$U_t = (\epsilon_{1,1,t},\epsilon_{1,2,t},...,\epsilon_{1,m,t},\epsilon_{2,1,t},\epsilon_{2,2,t},...\epsilon_{2,m,t},...,\epsilon_{n,m,t})'$
...
spectral_radius(res_d\$G) shows how many unit roots there are in the
CIGVAR system. The output of gvar_estimate, res_e, is an estimated
object of CIGVAR with estimated parameters, residuals and a series of
test statistics. res_e\$Summary gives the regression formats of each
country’s vector error correction model. The Johansen test results show
that the cointegration rank is 3 for all 4 countries. Therefore, the
specification of the cointegration rank has to adjust to achieve a
proper estimation.

The option of X in cigvar_data can be used to include exogenous variable
in the CIGVAR model.
...
res_d = cigvar_data(m=3,n=5,p=p,T=200,type="exog0",X=X,DFYflag=1,Ncommfakt = 0)
max(res_d$Y)
plot(ts(res_d$Y[,1:10]))
res_e = cigvar_estimate(res=res_d)
res_e$Summary

res_d = cigvar_data(m=3,n=5,p=p,T=200,type="exog1",X=X,DFYflag=1,Ncommfakt = 0)
max(res_d$Y)
plot(ts(res_d$Y[,1:10]))
res_e = cigvar_estimate(res=res_d)
res_e$Summary
...
CIGVARSelecte = cigvar_select_(res=res_d,L_V=c(4,4))
...
The example above shows how cigvar_select\_ is used for the model
...
cigvar_data function is used to generate data from a CIGVAR process with
global trends.
...
spectral_radius(res_e$GC)
```

For empirical applications it is recommended to follow the steps below:

- Use cigvar_data to generate a CIGVAR object.
- Replace the generated model data with the empirical data.
- Run a model selection procedure to determine the specification of the
  model if necessary.
- Estimate the selected model.
...
$m$-vector of the corresponding foreign variables with
$y_{i,t}^{*}=(y_{i,1,t}^{*},y_{i,2,t}^{*},...,y_{i,m,t}^{*})'$,
$i=1,2,...,n$. $m$ is the number of variables; $n$ is the number of
countries. Stacking all $n$ country models together we have MRGVAR in
its global form:

$$Y_{t}=C_{S_{t}}+\sum_{l=1}^{p}G_{l,S_{t}}y_{t-l}+\epsilon_{S_{t}}$$
with $\epsilon_{S_{t}}\sim N(0,\Sigma_{S_{t}})$. $Y_t$ is an (mn)
vector. $c_{S_t}$ is a constant (mn) vector depending on the state of
each country at time $t$.
...
The following sample codes demonstrate how mrgvar_data can be used as a
...
res_d <- mrgvar_data(m=2,n=10,p=p,TH=TH,T=200,S=2,SESVI=seq(1,20,2),type="const")
max(res_d$Y)
...
res_e$Summary
spectral_radius(res_e$Go[,,,1])   # check for the necessary condition of stationarity
spectral_radius(res_e$Go[,,,2])

if (! max(Mod(spectral_radius(res_e$Go[,,,1])),Mod(spectral_radius(res_e$Go[,,,2]))) > 1.001)  {

IRF_CB  = irf_mrgvar_cb(res=res_e,nstep=10,comb=NA,state=rep(1,10),irf="gen1",runs=20,conf=c(0.05,0.95))
IRF_g = plot_irf(IRF_CB[[1]], impulse=c(1,3),response=c(2,3,4,6),ncol=2)    #IRF
#IRF_g = plot_irf(IRF_CB[[2]])   # accumulated IRF

}
```

mrgvar_estimate is used to estimate the parameters of an MRGVAR object
with provided data. irf_mrgvar_cb and girf_mrgvar_rm_cb generate regime
...
$y_{i,t}^{*}=(y_{i,1,t}^{*},y_{i,2,t}^{*},...,y_{i,m,t}^{*})'$,
$i=1,2,...,n$. $m$ is the number of variables; $n$ is the number of
countries. The country model in level of the variables is

$$y_{i,t}  =  c_{i,S_{i,t}}+\sum_{l=1}^{p_{i,S_{i,t}}}B_{i,l,S_{i,t}}y_{i,t-l}+\sum_{l=1}^{q_{i,S_{i,t}}}A_{i,l,S_{i,t}}y_{i,t-l}^{*}+\epsilon_{i,S_{i,t}}$$
Stacking all $n$ country models together we have the MRCIGVAR in its
global form:
$$Y_{t}=c_{S_{t}}+\sum_{l=1}^{P}G_{l,S_{t}}Y_{t-l}+\epsilon_{S_{t}}$$
...
The sample codes below show how to use mrcigvar_data as a data
generating process to generate data from MRCIGVAR processes with a
variety of specifications. With the data provided, mrcigvar_estimate
estimates the parameters in an MRCIGVAR object. mrcigvar_select\_ can be
used to select an MRCIGVAR model’s model specification.
...
res_d <- mrcigvar_data(m=3,n=5,p=p,TH=TH,T=T,S=2, SESVI=c(1,4,7,10,13),type="exog0",r=rep(1,5),DFYflag=0,Ncommfakt=1,X=XX)    ## m: number of variables, n: number of countries
...
res_d <- mrcigvar_data(m=3,n=5,p=p,TH=TH,T=T,S=2, SESVI=c(1,4,7,10,13),type="exog0",Ao=Ao,Bo=Bo,Co=Co,r = rep(1,5),DFYflag = 0,Ncommfakt=1,X=XX)    ## m: number of variables, n: number of countries
...
res_d <- mrcigvar_data(m=3,n=5,p=p,TH=TH,T=T,S=2, SESVI=c(1,4,7,10,13),type="exog1",Ao=Ao,Bo=Bo,Co=Co,r = rep(1,5),DFYflag = 0,Ncommfakt=1,X=XX)    ## m: number of variables, n: number of countries
...
shock in state S:
$v_{t,S}^{gf}=(w_{1},0,w_{2},0,...,w_{N},0)\epsilon_{t,S}$ with
$\sum_{i=1}^{N}w_{i}=1$. Denoting $w'=(w_{1},0,w_{2},0,...,w_{N},0)$, we
have $v_{t,S}^{gf}=w'\epsilon_{t,S}$ and the standard error of the
global financial shock is  
$\sqrt{w'\Sigma_{S}w}$.
...
$\Omega_{t}$ contains all information available up to time $t$. For time
period $t$ the simultaneous response is
...
We define coordinated financial stress shocks in state $S$:
$v_{t,S}^{cf}=(v_{1},0,v_{3},0,...,v_{2N-1},0)$, where $v_{i}$ is the
scale of the financial stress shock $\epsilon_{i,t}$. Following the idea
of generalized impulse response function, we define the impulse response
function for concerted shocks as follows

where $R_{h}$ are obtained recursively as
$$R_{h}=\sum_{l=1}^{p}G_{l,S}R_{h-1}\mbox{ with }R_{0}=I\mbox{ and }R_{l}=0\mbox{ for }j<0,$$

Defining the impulse response function in this way, the simultaneous
response of $y_{i,t}$ to the concerted shock $v_{t,S}^{cf}$ is the sum
of conditional expected value of $\epsilon_{i,t}$ given concerted shocks
of other countries plus the shock of the own country.
...
###
p        <- p_G
res_d    <- mrgvar_data(m=2,n=15,p=p,T=370,S=2,SESVI=c((1:15)*2-1),type="const")
res_d$Y  <- FSIIO
res_d$W  <- Wmat_G
res_d$TH <- t(as.matrix(TH_G))

res_e   <- mrgvar_estimate(res_d)
...
IRF_list <- plot_irf(IRF_CB[[1]],Names=names(res_d$Y),impulse=c(1),response=seq(1,30,2),ncol=5)
IRF_list <- plot_irf(IRF_CB[[2]],Names=names(res_d$Y),impulse=c(1),response=seq(1,30,2),ncol=5)
# FSI responses to USA IP shock
IRF_list <- plot_irf(IRF_CB[[1]],Names=names(res_d$Y),impulse=c(2),response=seq(1,30,2),ncol=5)
IRF_list <- plot_irf(IRF_CB[[2]],Names=names(res_d$Y),impulse=c(2),response=seq(1,30,2),ncol=5)
```
...
- irf = “consert0”: this type is applied with a weighting vector in a
  column of the ($n \times n$) matrix comb to generate the coordinated
  policy action. The shocks are scaled in standard deviations
- irf = “concert1”: this type is applied with a weighting vector in a
  column of the ($n \times n$) matrix comb to generate the coordinated
  policy action. The shocks are scaled in units,
...
IRF_list <- plot_irf(IRF_CB[[1]],Names=names(res_d$Y),IName=iNames,impulse=c(1),response=seq(1,30,2),ncol=5)
# IP responses to a global one unit FSI  shock
IRF_list <- plot_irf(IRF_CB[[1]],Names=names(res_d$Y),IName=iNames,impulse=c(1),response=seq(2,30,2),ncol=5)

# FSI responses to an EU one unit FSI shock
IRF_list <- plot_irf(IRF_CB[[1]],Names=names(res_d$Y),IName=iNames,impulse=c(3),response=seq(1,30,2),ncol=5)
# IP responses to a EU one unit FSI shock
IRF_list <- plot_irf(IRF_CB[[1]],Names=names(res_d$Y),IName=iNames,impulse=c(3),response=seq(2,30,2),ncol=5)

# FSI responses to a global one unit IP shock
IRF_list <- plot_irf(IRF_CB[[1]],Names=names(res_d$Y),IName=iNames,impulse=c(2),response=seq(1,30,2),ncol=5)
# IP responses to a global one unit IP  shock
IRF_list <- plot_irf(IRF_CB[[1]],Names=names(res_d$Y),IName=iNames,impulse=c(2),response=seq(2,30,2),ncol=5)
...
IRF_list <- plot_irf(IRF_CB[[1]],impulse=c(1),response=seq(1,30,2),ncol=5)
#x11()  # FSI responses to USA scaled IP shock  (2.5)
IRF_list <- plot_irf(IRF_CB[[1]],impulse=c(2),response=seq(1,30,2),ncol=5)
...
Global_Response_CB <- irf_global_response_cb(IRF_CB,comb_all)


IRF_list <- plot_irf(Global_Response_CB[[1]],Names=iNames,INames=iNames,impulse=c(1),response=c(1:8),ncol=4)
```
