...
For local development, clone the repository and either load or install
the package locally:
...
Legacy aliases are still available for backward compatibility, but new
code should use the refactored lowercase function names.
...
For multi-regime workflows, the package provides parallel families such
...
This repository is under active refactoring. The main goals of the
refactor are:
...
- Most files in `R/` are split-module outputs and should be edited
  carefully.
- The MkDocs site combines native pages in `docs/` with generated pages
  rendered from `README.Rmd` and selected files in `vignettes/`.
...
Bug reports and feature requests are best submitted through the GitHub
issue tracker:
